...
def _standardize_time_index(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if isinstance(idx, pd.DatetimeIndex):
        # Engine already returns a tz-aware UTC DatetimeIndex; just normalize tz.
        out.index = idx.tz_convert("UTC") if idx.tz is not None else idx.tz_localize("UTC")
    elif np.issubdtype(np.asarray(idx).dtype, np.number):
...
def _run_engine(carry_ret: pd.DataFrame, position: pd.DataFrame, cost_bps: float) -> pd.Series:
    """Portfolio net per day. position is the ALREADY-1-bar-shifted carry-unit
    weight per coin (sums to <=2 gross). We aggregate the per-coin engine output
    into one portfolio series so the cost is charged on every coin's |Δposition|
    exactly like bt.run does per element, then summed across coins.

    bt.run is 1-D; we apply it column-by-column (each coin already aligned/shifted
    by the caller) and sum, which reproduces:
        gross_t   = Σ_i pos_{i,t} * ret_{i,t}
        cost_t    = (cost_bps/1e4) * Σ_i |pos_{i,t} - pos_{i,t-1}|
    """
    ...
    port = np.zeros(len(aligned_pos), dtype=float)
    for col in aligned_pos.columns:
        # position is already 1-bar-shifted by the caller -> bt.run must NOT re-shift.
        port = port + bt.run(aligned_ret[col].values, aligned_pos[col].values, cost_bps)
    return pd.Series(port, index=aligned_pos.index, name="net")
...
def _engine_metrics(net: pd.Series, position: pd.DataFrame, ppy: float) -> dict[str, float]:
    ...
    try:
        raw = bt.metrics(net, ppy, position=None)
        out = {k: float(v) if v is not None else np.nan for k, v in raw.items()}
        # Engine's matrix-turnover is meaningless for a 2-D position; replace it
        # with the honest portfolio CARRY-UNIT (pair) turnover per bar:
        #   turnover_t = Σ_i |pos_{i,t} - pos_{i,t-1}|
        pos = position.reindex(net.index).fillna(0.0)
        pair_turn = pos.diff().abs().sum(axis=1).fillna(pos.abs().sum(axis=1))
        out["turnover"] = float(pair_turn.mean())
        return out
    except Exception as exc:
        out = _empty_metrics()
